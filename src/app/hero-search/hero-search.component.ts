...
export class HeroSearchComponent implements OnInit {
   ...
   *   You can also push values into that Observable by calling its next(value) method as 
   * the search() method does."
   */
  ...
  * through the cellular network data plan.
  *   Instead, the ngOnInit() method pipes the searchTerms observable through a sequence of 
  * RxJS operators that reduce the number of calls to the searchHeroes(), ultimately returning
  ...
  */
  ngOnInit(): void {
    this.heroes$ = this.searchTerms.pipe(
       ...
       * from the most recent HTTP method call. Results from prior calls are canceled and 
       * discarded.
